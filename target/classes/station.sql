...
select * from housedeal where no = 3500;
select * from houseinfo where no = 3500;


SELECT hi.no as no, hi.dong as dong, hi.AptName as aptName, hi.code as code, hi.jibun as jibun, hd.dealAmount as dealAmount, hi.lat, hi.lng, st.stationName
from houseinfo as hi
join housedeal as hd
on hi.no = hd.no
join station as st
